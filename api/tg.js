  ...
  try {
    ...
    if (cmd === '/start') {
      ...
      if (arg && validToken(arg) && hasStore() && (await linkChat(arg, chatId))) {
        ...
        reply =
          '<b>Это TideWatch</b>\n\n' +
          'Дежурный по рынкам: 100 монет и 72 акции США в одном месте, ' +
          'без биржевого жаргона.\n\n' +
          '<b>Спроси что угодно:</b>\n' +
          '/market — что сейчас с рынком\n' +
          '/top — кто растёт, а кто падает\n' +
          '/coin btc — разбор монеты\n' +
          '/stock nvda — цена акции\n\n' +
          '<b>Главное:</b> поставь на сайте алерт на цену — и он придёт сюда, ' +
          'даже когда браузер закрыт. Дальше можно просто заниматься своими делами.\n\n' +
          'Кнопка ниже откроет приложение прямо в Telegram.'
      }
    } else if (cmd === '/help') {
      reply = HELP
    } else if (cmd === '/alerts') {
      // Алерты заводятся на сайте: там виден график и понятно, от какой цены
      // отсчитывать. Бот — только доставка.
      reply =
        '<b>Оповещения о цене</b>\n\n' +
        'Ставишь цену на сайте — сообщение приходит сюда, в этот чат. ' +
        'Браузер при этом можно закрыть: цену сверяет наш сервер, а не открытая вкладка.\n\n' +
        '<b>Как включить:</b>\n' +
        '1. Открой сайт кнопкой ниже\n' +
        '2. Нажми на колокольчик у любой монеты или акции\n' +
        '3. Подключи Telegram — это одна кнопка\n' +
        '4. Укажи цену, при которой тебя дёрнуть\n\n' +
        'Дальше можно просто заниматься своими делами.'
    } else if (cmd === '/channel') {
      reply =
